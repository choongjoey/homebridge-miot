...
class MiCloud {
  ...
  async loginTwoFa(notificationUrl, emailCode) {
    this.logger.debug(`(MiCloud) Starting 2FA email authentication flow`);

    // Step 1: Open notificationUrl (authStart)
    const context = await this._twoFaAuthStart(notificationUrl);

    // Step 2: Fetch identity options (list)
    const { identitySession, flag } = await this._twoFaFetchIdentityList(context);

    // Step 3: Send email verification code
    await this._twoFaSendEmailTicket(context, identitySession);

    // Step 4: Verify the email code
    const finishLocation = await this._twoFaVerifyEmail(context, identitySession, emailCode, flag);

    // Step 5-8: Follow redirect chain to extract ssecurity and serviceToken
